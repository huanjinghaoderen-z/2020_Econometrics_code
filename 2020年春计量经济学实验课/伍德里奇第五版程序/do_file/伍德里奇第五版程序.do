...
global path_Wooldridge "C:\Users\jiacheng pan\Desktop\2020年春计量经济学实验课\伍德里奇第五版程序"  // Add your local directory here
...
if $delimit == 1 /// // Use three slashes "///" to allow a command to span more than one line
{
...
if $chapter_4_linear_restrictions == 1 ///
{
...
if $chapter_7_C6 == 1 ///
{
	...
	*第四小题
	...
	test 1.urban#c.educ 1.urban#c.age 1.urban#c.agesq 1.urban#c.electric 1.urban#c.tv

	*采用公式计算出来的邹至庄F值同Stata计算的不同，问题在哪里？
	reg children educ age agesq urban electric tv if urban == 1
	scalar rss_urban = e(rss)

	reg children educ age agesq urban electric tv if urban == 0
	scalar rss_nonurban = e(rss)

	reg children educ age agesq urban electric tv
	scalar rss_pooled = e(rss)

	display ((rss_pooled-rss_urban-rss_nonurban)*4346)/((rss_urban+rss_nonurban)*5)

	reg children c.educ#i.urban c.age#i.urban c.agesq#i.urban c.electric#i.urban c.tv#i.urban
}

if $chapter_8_4_8_5 == 1 ///
{
	use "HPRICE1.dta",clear
...
if $chapter_15_1_5 == 1 ///
{
	...
	*ssc install estout
	esttab ols iv, se r2 /// //选择项“se”表示在括弧中显示标准误;
	mtitle /// //选择项“mtitle”表示使用模型名称(model title)作为表中每列的标题(默认使用被解释变量作为标题)
	star(* 0.1 ** 0.05 *** 0.01) // 选择项“star(* 0.1 ** 0.05 *** 0.01)”表示以星号表示显著性水平。
}
*   =====================================================================
